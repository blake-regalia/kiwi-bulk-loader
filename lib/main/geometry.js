...
Promise.all(process.argv.slice(2).map(p_arg => (async () => {
	...
	console.info(`updating geometries from ${s_basename}...`.blue);
	...
	await y_client.query(`copy ${s_table_tmp} from '${p_geoms}' (delimiter '\t', header false, format csv)`);

	// do updates
	let n_updates = (await y_client.query(`
		update nodes
		set gvalue = ${s_table_tmp}.gvalue
		from ${s_table_tmp}
