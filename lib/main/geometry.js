...
// well known text option
let b_option_wkt = argv.wkt;

// each file
Promise.all(argv._.map(p_arg => (async () => {
	// open connection to database
	...
	if(b_option_wkt) s_geom_cast = `st_geomfromewkt(${s_geom_cast})`;
	...
	let n_updates = (await y_client.query(`
		...
		where nodes.ntype = 'uri'
			and nodes.svalue = ${s_table_tmp}.gvalue
	`)).rowCount;
