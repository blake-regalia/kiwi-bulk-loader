...
function* snowflake(i_datacenter) {
	...
	for(;;) {
		...
		if(t_now < t_prev) throw new Error('system clock changed while script was running');
...
class tuples_triples_ssn extends tuples {
	constructor(h_fields, h_extras) {
		...
		// for(let s_key in h_fields) {
		// 	if(null === h_fields[s_key]) {
		// 		h_fields[s_key] = new extra_where(null);
		// 	}
		// }
...
class blank_node {
	...
	add(h_triple, k_handler, b_subject=false) {
		// add handler to list
		this.handlers.push([k_handler, b_subject]);

		// add triple to blank node's set
		return this.set.add(h_triple);
	}
...
class kiwi_loader {
	constructor(i_datacenter) {
		...
		Object.assign(this, {
			...
			// triples_ssn_pi_osl: new tuples_triples_ssn({
			// 	id: new field_id(fg_snowflake),
			// 	p_id: HPF_STRING,
			...
			// triples_ssn_ps_osl: new tuples_triples_ssn_ps({
			// 	id: new field_id(fg_snowflake),
			// 	o_ntype: new extra_where('string'),
			// 	o_ltype: HPF_STRING,
			// 	o_lang: new extra_where(null),
	...
	push_blank_node(kt, h_triple, k_handler, b_subject) {
		let {
			// terms_sn: kq_terms_sn,
		...
		if(s_label in h_blanknodes) {
			// add quad to set
			h_blanknodes[s_label].add(h_triple, k_handler, b_subject);
		}
		// blank node not yet exists
			...
			kt_blank_node.add(h_triple, k_handler, b_subject);
		...
		return new Promise((fk_parse) => {
			...
			graphy.ttl.deserializer(ds_input, {
				data(h_triple) {
					...
					// triple handler
					let k_handler = new triple_handler(k_self, h_triple);
					...
					if(kt_subject_prev !== kt_subject) {
						...
						// blanknode
							...
							// push blank node
							k_self.push_blank_node(kt_subject, h_triple, k_handler, true);
						}
					...
					if(kt_object.isBlankNode) {
						k_self.push_blank_node(kt_object, h_triple, k_handler, false);
					}
